...
export function FirstVisitInterstitial() {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[100] bg-foreground/85 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto"
    >
      <div className="relative w-full max-w-5xl bg-card rounded-3xl shadow-2xl p-6 sm:p-10 my-8">
        <button
          type="button"
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">
            Pick the option that fits. You can always come back to the homepage from here.
          </p>
        </div>
          ...
          {TILES.map((tile) => {
            ...
            return (
              <button
                ...
                className="group text-left flex flex-col gap-3 p-6 rounded-2xl border border-border bg-background hover:bg-primary/5 hover:border-primary hover:-translate-y-0.5 hover:shadow-lg transition-all"
              >
                <div className="w-12 h-12 rounded-xl bg-secondary group-hover:bg-primary group-hover:text-primary-foreground flex items-center justify-center transition-colors">
                  <Icon className="w-6 h-6" />
                </div>
                <p className="font-bold text-base leading-snug">{tile.title}</p>
                <p className="text-sm text-muted-foreground leading-relaxed flex-1">{tile.body}</p>
