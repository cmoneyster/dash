...
// Cached DB secret so we don't hit the DB on every webhook call.
// Invalidated after 5 minutes so a regenerate takes effect promptly.
let _cachedSecret: { value: string | null; expiresAt: number } | null = null;
async function getWebhookSecret(): Promise<string | null> {
  const now = Date.now();
  if (_cachedSecret && now < _cachedSecret.expiresAt) return _cachedSecret.value;
  const [row] = await db.select({ smsWebhookSecret: eventSettingsTable.smsWebhookSecret }).from(eventSettingsTable).where(eq(eventSettingsTable.id, 1));
  const dbSecret = row?.smsWebhookSecret?.trim() || null;
  const envSecret = process.env.SMS_WEBHOOK_SECRET?.trim() || null;
  const value = dbSecret ?? envSecret;
  _cachedSecret = { value, expiresAt: now + 5 * 60 * 1000 };
  return value;
}
...
async function handleInbound(req: Request, res: Response, raw: Record<string, unknown>): Promise<void> {
  try {
    ...
    if (!secret) {
      // Refuse to act as an open relay. A secret is auto-generated by
      // /admin/sms-settings on first load — if none exists yet, push
      // mode hasn't been configured.
