...
3. **Run backfill.** Click "Run backfill now" on the SMS Settings page. The api-server log will emit one `[ejoin] inbound fetch` line per fetch and one `[sms-inbox] ingest` line per message ingested with `status` (the raw IngestResult: stored / dedup / opted-out / blocked / owner-reply-relayed / owner-reply-rejected / skipped-empty), an operator-readable `outcome` token (`stored-matched` / `stored-unmatched` / and the same value as `status` for every other case), `inquiryId` (matched id or null), and `gid` (gateway message id). The two-field shape exists so quick log scans can grep for `outcome:"stored-unmatched"` to find Unmatched-bound messages without inferring it from `inquiryId:null`. Copy the gid into the diagnostics endpoint's `sampleRows` to compare what backfill saw vs. what the gateway is currently serving.
